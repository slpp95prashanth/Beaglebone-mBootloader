...
#endif
static int MyReadFileAndCheck(unsigned char *src, void *dest, size_t size, size_t *RPOS)
{
    printf("src=%p, dest=%p, size=%u, RPOS=%p\n", src, dest, size, RPOS);
  if (size == 0)
    return 0;
...
int lzma_inflate(unsigned char *source, int s_len, unsigned char *dest, int *d_len, void *PTR)
{
  ...
  if (!MyReadFileAndCheck(source, properties, sizeof(properties),&rpos))
  {
  ...
  /* Read uncompressed size */
  {
    ...
    printf("outSizeFull=%u, outSizeHigh=%u\n", outSizeFull, outSizeHigh);
...
#ifdef DEBUG_ENABLE_BOOTSTRAP_PRINTF
    printf("WRONGproperties");
#endif
    return 7;
  }
  void* (*malloc_ptr)(size_t) = PTR;
  state.Probs = (CProb *)(*malloc_ptr)(LzmaGetNumProbs(&state.Properties) * sizeof(CProb));
